...
export default function IdeasPage() {
  ...
  return (
    <div>
      ...
      <p className="page-subtitle">All discovered and validated startup ideas</p>
      ...
      {loading ? (
        <div className="ideas-grid">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="card"><div className="skeleton" style={{ height: 150 }} /></div>
          ))}
        </div>
      ) : ideas.length > 0 ? (
        <div className="ideas-grid">
          {ideas.map((idea) => (
            <div key={idea.id} className="card idea-card">
              <Link href={`/ideas/${idea.id}`} style={{ color: 'inherit' }}>
        ...
        <div className="empty-state">
          ...
          <div className="empty-state-title">No ideas found</div>
          <div className="empty-state-text">
            {statusFilter || categoryFilter || search
              ? 'Try changing your filters'
              : 'Run a pipeline to generate startup ideas'}
          </div>
        </div>
      )}
    </div>
